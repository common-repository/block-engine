...
function block_engine_getNodeindex(elem) {
	return Array.prototype.slice.call(elem.parentNode.children).indexOf(elem);
}

function block_engine_handleTabEvent(tab) {
	const parent = tab.closest(".wp-block-block-engine-tabbed-content-holder");

	const isVertical = parent.classList.contains("vertical-holder");

	const activeStyle = parent
		.querySelector(
			`.wp-block-block-engine-tabbed-content-tab-title-${
				isVertical ? "vertical-" : ""
			}wrap.active`
		)
		.getAttribute("style");
	...
	block_engine_getSiblings(tab, (elem) =>
	...
	const tabContentContainer = Array.prototype.slice
		...
		.filter((elem) =>
			elem.classList.contains("wp-block-block-engine-tabbed-content-tabs-content")
		)[0];

	Array.prototype.slice
		.call(
			tabContentContainer.getElementsByClassName(
				"wp-block-block-engine-tabbed-content-tab-content-wrap"
			)
		)
		.forEach((tabContent, i) => {
			if (block_engine_getNodeindex(tab) === i) {
				tabContent.classList.add("active");
				tabContent.classList.remove("block-engine-hide");
				let flickityInstances = Array.prototype.slice.call(
					tabContent.querySelectorAll(".block_engine_image_slider")
				);
				...
				tabContent.classList.add("block-engine-hide");
			}
		});
	...
	Array.prototype.slice
		.call(
			tabContentContainer.getElementsByClassName(
				"wp-block-block-engine-tabbed-content-accordion-toggle "
			)
		)
		.forEach((accordionToggle) => {
			if (block_engine_getNodeindex(accordionToggle) / 2 === block_engine_getNodeindex(tab)) {
...
Array.prototype.slice
	.call(
		document.getElementsByClassName("wp-block-block-engine-tabbed-content-tab-title-wrap")
	)
	.forEach((instance) => {
		instance.addEventListener("click", function () {
			block_engine_handleTabEvent(instance);
		});
...
Array.prototype.slice
	.call(
		document.getElementsByClassName(
			"wp-block-block-engine-tabbed-content-tab-title-vertical-wrap"
		)
	)
	.forEach((instance) => {
		instance.addEventListener("click", function () {
...
function block_engine_switchFocusToTab(index, tabBar) {
...
(function () {
	const displayModes = Array.prototype.slice
		.call(document.getElementsByClassName("wp-block-block-engine-tabbed-content"))
		.map((block) => {
			let dm = [];
			const classNamePrefix = "wp-block-block-engine-tabbed-content";
				...
				.forEach((instance, i) => {
					if (
							...
							Array.prototype.slice
								...
								.forEach((child, j) => {
									if (Math.floor(j / 2) === activeTabs[activeTabs.length - 1]) {
										child.classList.add("active");
										child.classList.remove("block-engine-hide");
									} else {
										child.classList.remove("active");
										if (j % 2 === 1) {
											child.classList.add("block-engine-hide");
										}
									}
								});
						...
						if (displayModes[i][transitionTo - 1] === "horizontaltab") {
							...
							block_engine_switchFocusToTab(newActiveTab, tabBar);
						}
					}
				});
			transitionTo = 0;
...
function block_engine_hashTabSwitch() {
		...
		if (
			...
			if (
				...
				setTimeout(() => {
					...
					targetElement.classList.remove("block-engine-hide");
				...
				Array.prototype.slice.call(tabBar.children).forEach((tab, i) => {
					...
					if (i === tabIndex) {
						...
						tabContents[i].classList.remove("block-engine-hide");
						...
						tabContents[i].classList.add("block-engine-hide");
						if (
							probableAccordionToggle &&
							probableAccordionToggle.classList.contains(
								`${classNamePrefix}-accordion-toggle`
				...
				if (
					...
					block_engine_switchFocusToTab(tabIndex, tabBar);
				}
			}
		}
	}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	block_engine_hashTabSwitch();
});

window.onhashchange = block_engine_hashTabSwitch;

Array.prototype.slice
	.call(
		document.getElementsByClassName("wp-block-block-engine-tabbed-content-tabs-content")
	)
	.forEach((container) => {
		Array.prototype.slice
			.call(
			...
			.forEach((accordionToggle, i) => {
				accordionToggle.addEventListener("click", () => {
					...
					accordionToggle.nextElementSibling.classList.toggle("block-engine-hide");
				});
			});
	});
...
Array.prototype.slice
	.call(
		document.getElementsByClassName("wp-block-block-engine-tabbed-content-tab-holder")
	)
	.forEach((tabBar) => {
		let tabBarIsBeingDragged = false;
		let oldScrollPosition = -1;
