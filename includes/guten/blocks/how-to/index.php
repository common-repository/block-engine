...
function block_engine_generateISODurationCode($inputArr){
...
function block_engine_convert_to_paragraphs($string){
    if($string == ''){
        return '';
    }
    else{
...
function block_engine_render_how_to_block($attributes){
    extract($attributes);
    ...
    if($advancedMode && $includeSuppliesList){
        ...
        if(count($supplies) > 0){
            ...
            $header .= ' class="block_engine_howto-supplies-list">';
    ...
    if($advancedMode && $includeToolsList){
        ...
        if(count($tools) > 0){
            ...
            $header .= ' class="block_engine_howto-tools-list">';
    ...
    $ISOTotalTime = block_engine_generateISODurationCode($totalTime);
    ...
    if($useSections){
        $stepsDisplay =  ($sectionListStyle == 'ordered' ? '<ol' : '<ul') .
                            ' class="block_engine_howto-section-display">';
        foreach($section as $i => $s){
            $stepsDisplay .= '<li class="block_engine_howto-section"><h3>' . $s['sectionName'] . '</h3>' .
            ($sectionListStyle == 'ordered' ? '<ol' : '<ul') . ' class="block_engine_howto-step-display">';
            ...
            foreach($s['steps'] as $j => $step){
                ...
                $stepsDisplay .= '<li class="block_engine_howto-step"><h4 id="'.$step['anchor'].'">'
                    . $step['title'].'</h4>' . ($step['stepPic']['url'] != '' ? '<img class="block_engine_howto-step-image" src="' .$step['stepPic']['url']. '">' : '')
                    . block_engine_convert_to_paragraphs($step['direction']) . PHP_EOL;
                ...
                if ($step['tip'] != ''){
                    $stepsDisplay .= block_engine_convert_to_paragraphs($step['tip']);
        ...
        $stepsDisplay .=  ($sectionListStyle == 'ordered' ? '<ol' : '<ul') .
                            ' class="block_engine_howto-step-display">';
        if(count($section) > 0){
            foreach($section[0]['steps'] as $j => $step){
                $stepsDisplay .= '<li class="block_engine_howto-step"><h4 id="'.$step['anchor'].'">'
                        . $step['title'].'</h4>' . ($step['stepPic']['url'] != '' ? 
                        '<img class="block_engine_howto-step-image" src="' .$step['stepPic']['url']. '">' : '') . block_engine_convert_to_paragraphs($step['direction']);
                $stepsCode .= '{"@type": "HowToStep",'. PHP_EOL
                            . '"name": "'.$step['title'].'",' . PHP_EOL
                            . ($advancedMode ? '"url": "'.get_permalink().'#'.$step['anchor'].'",' . PHP_EOL
                            . ($step['hasVideoClip'] ? '"video":{"@id": "'.$step['anchor'].'"}' : '') . PHP_EOL : '')
    ...
    return '<div class="block_engine_howto" id="block_engine_howto_'.$blockID.'"><h2>'
                . $title . '</h2>' . block_engine_convert_to_paragraphs($introduction) . $header .
                ($advancedMode ? ($videoURL == '' ? '' : $videoEmbedCode) 
                . '<p>' . $costDisplayText . $costDisplay . '</p>'
                . $timeDisplay : '') . $stepsDisplay .   
                '<h2>' . $resultIntro . '</h2>' . 
                ($finalImageURL == '' ? '' : '<img class="block_engine_howto-yield-image" src="' .$finalImageURL. '">') .
                block_engine_convert_to_paragraphs($howToYield) .
            '</div>' . $JSONLD;
}

function block_engine_register_how_to_block(){
    if ( function_exists( 'register_block_type' ) ) {
        require dirname(dirname(__DIR__)) . '/defaults.php';
        register_block_type('block-engine/how-to', array(
            'attributes' => $defaultValues['block-engine/how-to']['attributes'],
            'render_callback' => 'block_engine_render_how_to_block'));
    }
}

add_action('init', 'block_engine_register_how_to_block');